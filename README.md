...
| 📥 **קליטה חכמה** | **זורקים קובץ למקום אחד** — המערכת מזהה לבד גוף/סוג/שנה/מועד **ומתייקת אוטומטית**; המשתמש רק מפקח. זיהוי היברידי: כללים מקומיים + **Claude (ראייה)** לסרוקים ולעברית. חברה שלא ברשימה? יצירת גוף בלחיצה. קובץ כפול? מזוהה ולא מועלה שוב |
...
2. **Claude (ראייה)** — קורא **מסמכים סרוקים/מצולמים ועברית** ומחלץ גוף/סוג/שנה/מועד בדיוק גבוה. מופעל רק כשהכללים לא בטוחים.

כדי להפעיל את שלב Claude, צריך מפתח API:
```bash
cp .env.example .env      # ובקובץ .env: ANTHROPIC_API_KEY=sk-ant-...
```
מפתח מ-[console.anthropic.com](https://console.anthropic.com) → API Keys. **בלי מפתח** המערכת עובדת עם הכללים בלבד. ⚠️ בשלב Claude המסמך נשלח ל-API של Anthropic (לא משמש לאימון).
