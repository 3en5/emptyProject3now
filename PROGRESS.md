...
|---|---------|----------------|-------|
| 63 | **מסך "מוכנות לרו״ח" (frontend, השלמה ל-#62)** — עמוד ייעודי לפי הסקיצה שאושרה: hero עם מונה have/total+מד התקדמות+מיני-סטטיסטיקות, כפתור ייצוא CSV (קישור להורדה), מתג "הכול/רק החסרים" (מסתיר פריטים שהתקבלו + קבוצות מוכנות לגמרי), "🔄 בנה מרשימת אשתקד" (`carry-forward`, עם הודעת "נוספו N פריטים"/"אין מה לגלגל"), קבוצות לפי גוף (`ReadinessGroup`) מסודרות חסרים→מוכנים✓→הסתיימו, כל שורת מסמך עם chip יש/חסר+העלאה (`/:id/upload`, לא `PUT`)+מחיקה, "➕ הוסף פריט" בסוף כל קבוצה, "סיימתי עם הגוף הזה"/"↩ החזר לצפי" (PUT מלא לגוף — נשלף קודם `GET /api/entities` כי אין COALESCE בעריכת גוף). עמוד עצמאי (שולף בעצמו `readiness`+`entities`, כמו `ComparisonPage`), מכבד `useReadOnly()`. **תצוגת צ׳קליסט צפוף** — כל מסמך שורה אחת (שם + chip יש/חסר + ›), ולחיצה פותחת **מודאל רחב** (`ReadinessItemModal`) עם הפירוט וכל הפעולות (העלאה/החלפה/פתיחת קובץ/מחיקה + תקציר/סכומים מ-GPT אם יש); סגירה ב-X/רקע/Esc. הכל נשאר גלוי (כולל רכב/רישיונות — הוצאות עסק), שום דבר לא מוסתר. עיצוב: `styles/readiness.css`. ניווט: כפתור מיד אחרי "עמוד הבית". **אומת בהרצה אמיתית + צילומי מסך** (רשימה + מודאל). 260 טסטים (144 API + 92 רכיבים + 24 E2E), כולל E2E ייעודי (צ׳קליסט→מודאל) | `pages/ReadinessPage.jsx`, `components/ReadinessGroup.jsx`, `components/ReadinessItemModal.jsx`, `styles/readiness.css`, `index.css`, `App.jsx`, `Navigation.jsx`, `test/readiness.test.jsx`, `e2e/smoke.spec.js`, `README.md`, `INDEX.md` | 2026-07-09 |
| 62 | **מוכנות לרו״ח (readiness) — תצוגה לפי גוף** — `GET /api/readiness/:year` מציג, לכל גוף פיננסי, אילו מסמכי `documents` של השנה יש להם קובץ (✅ התקבל) ומה חסר (🔴), עם סיכום כללי; קיבוץ עם חוסרים קודם, ובתוך כל גוף — חסר קודם. גוף פעיל בלי מסמכים באותה שנה עדיין מופיע (רשימה ריקה) כדי שאפשר יהיה להוסיף לו פריטים. **לא נוצרה טבלה/מושג משימה חדש** — שכבת תצוגה בלבד מעל `documents` הקיימת (`file_path` קיים/ריק = יש/חסר), באותו רוח כמו `comparison.js`. `POST /api/readiness/:year/carry-forward` בונה סלוטים ל-:year מכל מסמך של year-1 של גוף עדיין פעיל, שאין לו כבר סלוט השנה — אידמפוטנטי. 144 טסטי API ירוקים (133→144, +11 חדשים); רכיבי React/E2E לא נגעו (לא הורצו בסבב הזה). Frontend ייעודי (מסך "מוכנות לרו״ח") מומש ב-#63 | `backend/routes/readiness.js`, `backend/app.js`, `backend/test/readiness.test.js`, `INDEX.md` | 2026-07-09 |
| 61 | **תיקון: מחזור המשימות מעוגן בשנת המסמך + חיווי השלמה גם בלי היסטוריה** — באג שהמשתמש דיווח: העלאת מסמך מ-2023 לא יצרה משימת המשך ולא חיווי "הושלם" ל-2023. הסיבה: `applyChecklistCycle` היה מעוגן בשנה הקלנדרית (`new Date().getFullYear()`) ולא בשנת המסמך, וגם לא נרשמה משימה מושלמת כשלא הייתה היסטוריה קודמת. תוקן: המחזור מעוגן ב-`doc.year` (fallback לשנה הנוכחית), ולכל מסמך נרשמת משימה מושלמת עבור שנתו (מסדרת השנה הקודמת אם קיימת, אחרת נגזרת מהמסמך — `buildCompletedFromDoc`), פלוס משימת המשך ל-Y+1. חיווי `IntakeBox` שונה לפי-שנה. פוצל `intake.test.js` (חצה 500 שורות) → `checklistCycle.test.js`. לקח #10. בוצע בסוכן Sonnet + אימות E2E מרכזי (כולל תיקון 2 טסטי E2E שהניחו עיגון-שנה-נוכחית). 240 טסטים (133 API + 84 רכיבים + 23 E2E) | `backend/checklistRollover.js`, `backend/routes/documents.js`, `IntakeBox.jsx`, `backend/test/checklistRollover.test.js`, `backend/test/checklistCycle.test.js`, `intake.test.js`, `intake.test.jsx`, `smoke.spec.js`, `LESSONS.md` | 2026-07-09 |
| 60 | **בורר שנה דינמי (לפי הנתונים, לא רשימה קשיחה)** — עד עכשיו בוררי השנה (`ChecklistPage`, `ComparisonPage`) היו חלון קשיח של 4 שנים `[current+1..current-2]`, אז שנים ישנות (2019–2023) פשוט לא היו זמינות. נוסף `GET /api/system/years` שמאחד חלון ברירת מחדל (`current-7..current+1` — 7 שנים = תקופת שמירת מסמכים לפי דיני המס) עם כל שנה הקיימת בפועל ב-`documents`/`annual_checklist`. hook חדש `frontend/src/hooks/useYears.js` מזין את שני הבוררים (fallback לחלון ברירת מחדל בכשל). התוצאה: מסמך משנה ישנה (למשל 2015) מוסיף את שנתו לבורר אוטומטית — בדיוק המנגנון שהמשתמש ביקש. בוצע בשני סוכני Sonnet מקבילים (backend/frontend) + E2E מרכזי. 234 טסטים (128 API + 84 רכיבים + 22 E2E) | `backend/routes/system.js`, `backend/test/system.test.js`, `frontend/src/hooks/useYears.js`, `ChecklistPage.jsx`, `ComparisonPage.jsx`, `checklist.test.jsx`, `comparison.test.jsx`, `smoke.spec.js`, `INDEX.md` | 2026-07-09 |
...
| 58 | **בורר שנה במשימות שנתיות** — עד עכשיו `ChecklistPage` הציג רק את השנה הנוכחית, בלי דרך לצפות במשימות של שנים אחרות (קודמות/עתידיות). נוסף בורר שנה (`YEARS` — הבאה/נוכחית/שתי הקודמות): השנה הנוכחית ממשיכה להגיע מה-`checklist` prop הגלובלי (כמו קודם — גם `Dashboard` תלוי בו), ושנה אחרת נשלפת עצמאית מ-`GET /api/checklists/year/:year` (קיים כבר ב-backend, פשוט לא נוצל). הוספת/עריכת משימה בזמן צפייה בשנה אחרת יוצרת אותה תחת אותה שנה (לא תמיד השנה הנוכחית). 208 טסטים (111 API + 77 רכיבים + 20 E2E) | `ChecklistPage.jsx`, `checklist.test.jsx`, `smoke.spec.js`, `INDEX.md` | 2026-07-08 |
| 57 | **תיקון: קובץ שהוצמד לא סימן "הוגש" (status נשאר "ממתין")** — קליטת קובץ (תיוק לסלוט/מסמך חדש ב-`/intake`, החלפת קובץ ב-`/:id/upload`) עדכנה `file_path`/`auto_filed` אבל מעולם לא נגעה ב-`documents.status`, אז המסמך נשאר "⏳ ממתין" גם אחרי שהקובץ בפועל הוצמד. באג נלווה: `PUT /:id` דרס `status` בלי `COALESCE` (בניגוד לכל שדה אופציונלי אחר בראוט), אז "אשר ושמור" ב-`IntakeBox` (שלא שולח status) איפס אותו בשקט ל-NULL. תוקן: `status='submitted'` נקבע במפורש בכל נקודה שקובץ אמיתי מצטרף; `PUT` עבר ל-`COALESCE` על `status`. לקח #8. 204 טסטים | `routes/documents.js`, `intake.test.js`, `api.test.js`, `LESSONS.md`, `INDEX.md` | 2026-07-08 |
| 56 | **עבור מי המסמך (אני / בן-זוג)** — GPT מזהה `personName` (השם שמופיע על המסמך — עובד/ת, מבוטח/ת, בעל/ת חשבון) ומתאים אותו ל-`FINANCE_USER_NAME`/`FINANCE_SPOUSE_NAME` (`.env`, אופציונלי) דרך `backend/ownerMatch.js` (טהורה) → `ownerGuess`. עמודה חדשה `documents.owner`. אותו דפוס "זיהוי+אישור" כמו כל שאר הזיהויים: בתיבת הקליטה שדה "עבור מי" ממולא מה-guess עם רמז לשם שזוהה; בכרטיס המסמך תג בעלים בכותרת המקופלת + בורר לשינוי ישיר + בורר בתיבת הניתוח. בלי הגדרת שמות ב-`.env` — תמיד `null` (בחירה ידנית בלבד, לא מנחשים). 203 טסטים (110 API + 74 רכיבים + 19 E2E) | `gpt.js`, `ownerMatch.js`, `understand.js`, `routes/documents.js`, `db/schema.sql`, `db/init.js`, `.env.example`, `constants/owner.js`, `IntakeBox.jsx`, `DocumentPage.jsx`, `styles/cards.css`, `ownerMatch.test.js`, `understand.test.js`, `intake.test.js`, `intake.test.jsx`, `analyze.test.jsx`, `smoke.spec.js` | 2026-07-08 |
| 55 | **סוג גוף חדש: "תרומה" 🎗️** — נוסף ל-`ENTITY_TYPES`, קטגוריות ייעודיות (תרומה מוכרת סעיף 46 / חד-פעמית / הוראת קבע), אייקון בכל מפות התווית (`EntityList`, `Dashboard`, `ReportsPage`, `EntitiesPage`), ו-`entityType` ב-schema של GPT כדי שקבלות תרומה יזוהו נכון. תיעוד ב-`REPORTS.md` (סעיף 46). 198 טסטים | `constants/entityTypes.js`, `EntityForm.jsx`, `EntityList.jsx`, `Dashboard.jsx`, `ReportsPage.jsx`, `EntitiesPage.jsx`, `gpt.js`, `REPORTS.md`, `components.test.jsx`, `understand.test.js` | 2026-07-08 |
| 54 | **כרטיסים מתקפלים (accordion) — מסמכים וגופים פיננסיים** — כרטיס מקופל כברירת מחדל: כותרת בלבד (שם/סטטוס/גוף/🤖 למסמכים; שם/קטגוריה לגופים), לחיצה פותחת/סוגרת. פותח אוטומטית כשמריצים ניתוח (כדי לראות את תיבת ההצעות). 197 טסטים (97 API + 70 רכיבים + 19 E2E) | `DocumentPage.jsx`, `EntityList.jsx`, `styles/cards.css`, `components.test.jsx`, `analyze.test.jsx`, `readonly.test.jsx`, `smoke.spec.js` | 2026-07-08 |
| 53 | **תצוגת מסמכים כרשימה במקום רשת קוביות** — `.documents-list` (עמודה אחת, שורה לכל מסמך) במקום `.documents-grid` (רשת רב-עמודתית); `.entities-grid` נשאר רשת (לא התבקש שינוי שם) | `DocumentPage.jsx`, `styles/cards.css`, `styles/misc.css` | 2026-07-08 |
| 52 | **השלמה אוטומטית של משימה שנתית עקב מסמך שהתקבל** — מסמך שנקלט (למשל "טופס 106") מסומן כעת אוטומטית ב-V מול משימה שנתית תואמת ("איסוף טופס 106") — לא רק תיוק מסמכים, גם משימות outbound. התאמה לפי ביטוי מלא של סוג/שם המסמך **או** שם הגוף המנפיק בטקסט המשימה (`backend/checklistMatch.js`, טהורה). פועל גם מ-`/intake` וגם מ-`/:id/upload`. פיקוח: תג "🤖 הושלם אוטומטית עקב מסמך X" + "✓ אשר" בעמוד המשימות (אותו דפוס כמו `documents.auto_filed`); "↩️ החזר לממתין" מנקה את הקישור למסמך. הודעה בתיבת הקליטה. 182 טסטים (97 API + 66 רכיבים + 19 E2E) | `checklistMatch.js`, `routes/documents.js`, `routes/checklists.js`, `db/schema.sql`, `db/init.js`, `ChecklistPage.jsx`, `IntakeBox.jsx`, `styles/checklist.css`, `styles/intake.css`, `checklistMatch.test.js`, `intake.test.js`, `checklist.test.jsx`, `intake.test.jsx`, `smoke.spec.js` | 2026-07-08 |
