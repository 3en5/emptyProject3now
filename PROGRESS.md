...
- [ ] **עמוד חשבונות** — ה-API (`/api/accounts`) קיים, אין UI כלל
- [ ] **עריכה מלאה של מסמך/משימה** (לא רק סטטוס — גם שם/תאריך/גוף) דרך טופס

### שלב 2 — ניהול מסמכים מתקדם
- [ ] **רמזור/התראה לדוחות שמתקרב מועד הגשתם** (ערך גבוה — ליבת המטרה)
